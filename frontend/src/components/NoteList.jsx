...
const NoteList = ({ notes, onEdit, onDelete, onCreate }) => {
  if (!notes.length) return (
    <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-6 gap-3 md:gap-4">
      ...
      >
        <svg className="w-12 h-12 md:w-16 md:h-16 text-cyan-600 mb-2 md:mb-4 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
        </svg>
        <p className="text-cyan-600 font-semibold text-base md:text-lg">New Note</p>
      </div>
      <div className="col-span-full text-center py-12">
        <p className="text-gray-500 text-lg md:text-xl">No notes yet. Create your first note!</p>
      </div>
    </div>
  );
